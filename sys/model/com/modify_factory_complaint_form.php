...
if($myerror->getWarn()){
    ...
    if(isset($_GET['delid']) && $_GET['delid'] != ''){
        ...
        if($rtn){
            ...
            $myerror->ok('删除 Factory Complaint Form 成功!', 'com-search_factory_complaint_form&page=1');
        }else{
            $myerror->error('删除 Factory Complaint Form 失败!', 'com-search_factory_complaint_form&page=1');
        }
    }else{
        ...
        $formItems = array(
            ...
            'about' => array('title' => '有关SO/PO(如有)', 'type' => 'text', 'minlen' => 1, 'maxlen' => 50, 'value' => isset($mod_result['about'])?$mod_result['about']:''),
            ...
            'detail' => array('title' => '具体内容', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 1000, 'value' => isset($mod_result['detail'])?$mod_result['detail']:''),
            'fty_reply' => array('title' => '工厂回应/行动', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 1000, 'value' => isset($mod_result['fty_reply'])?$mod_result['fty_reply']:'', 'disabled' => 'disabled'),
            'reply_satisfaction' => array('title' => '回复满意度', 'type' => 'radio', 'options' => array(array(1,1),array(2,2),array(3,3),array(4,4),array(5,5)), 'value' => isset($mod_result['reply_satisfaction'])?$mod_result['reply_satisfaction']:''),
            'reply_remark' => array('title' => '备注', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 1000, 'value' => isset($mod_result['reply_remark'])?$mod_result['reply_remark']:''),
        ...
        if(!$myerror->getAny() && $goodsForm->check()){
            ...
            $reply_satisfaction = trim($_POST['reply_satisfaction']);
            $reply_remark = trim($_POST['reply_remark']);
            $in_date = $mod_date = dateMore();
            $mod_by = $_SESSION['logininfo']['aName'];

            $result = $mysql->q('update fty_complaint_form set fty_sid = ?, send_to = ?, form_date = ?, about = ?, remark = ?, detail = ?, reply_satisfaction = ?, reply_remark = ?, mod_date = ?, mod_by = ? where fc_id = ?', $pc_sid, $pc_attention, $form_date, $about, $remark, $detail, $reply_satisfaction, $reply_remark, $mod_date, $mod_by, $fc_id);
            if($result){
                ...
                $myerror->ok('修改 Factory Complaint Form 成功!', 'com-search_factory_complaint_form&page=1');
            }else{
