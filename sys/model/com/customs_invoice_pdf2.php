...
if (isset($_GET['vid']) && $_GET['vid'] != '') {
    ...
    //修改printed_date
    $mysql->q('update customs_invoice set printed_date = ? where vid = ?', dateMore(), $_GET['vid']);

    $result1 = $mysql->qone('select * from customs_invoice where vid = ?', $_GET['vid']);
    if (!$result1) {
        die('Error(1)');
    }
    ...
    $rs2 = $mysql->q('select * from customs_invoice_item where vid = ?', $_GET['vid']);
    if ($rs2) {
        $result2 = $mysql->fetch();
    } else {
        die('Error(2)');
    ...
    //找到調高度的方法了
    $pdf->Ln(1);
    $html = '<span align="right"><b>Commercial Invoice</b><span/>';
...
//product 的個數
    ...
    for ($i = 0; $i < count($result2); $i++) {
        //由于 customs invoice 是给海关看的，不需要显示完整的desc
        //这样做是因为如果换行多，只用字符数来判断，还是可能出现很多行
        //mod 20120911 因对方要求，现完整显示description
        /*
        if(substr_count($result2[$i]['description'], "\r\n") >= 2){
            $temp_result2_description = explode("\r\n", $result2[$i]['description']);
            $result2[$i]['description'] = $temp_result2_description[0]."\r\n".$temp_result2_description[1];
        }else{
            $result2[$i]['description'] = substr($result2[$i]['description'], 0, 70);
        }
        */
    ...
    */
    ...
    //20130816 去掉TOTAL，只显示EX-FACTORY TOTAL
