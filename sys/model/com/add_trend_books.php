...
$formItems = array(
    ...
    'trend_books_date' => array('title' => 'Date', 'type' => 'text', 'restrict' => 'date', 'required' => 1, 'value' => date('Y-m-d')),
...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    if( ((@$_FILES['photo']['type'] == 'image/jpg' || @$_FILES['photo']['type'] == 'image/jpeg' || @$_FILES['photo']['type'] == 'image/gif' || @$_FILES['photo']['type'] == 'image/pjpeg' || @$_FILES['photo']['type'] == 'image/png') && (@$_FILES['photo']['size'] / 1024) <= 500) || @$_FILES['photo']['name'] == '' ){
        ...
        if (@$_FILES['photo']['error'] > 0 && @$_FILES['photo']['error'] != 4){
            ...
            $result = $mysql->q('insert into trend_books set photo = ?, trend_books_date = ?, created_by = ?, mod_by = ?, in_date = ?, mod_date = ?', $photos, $trend_books_date, $created_by, $mod_by, $in_date, $mod_date);
            if($result){
                $myerror->ok($add_tip.'Add Trend Books success!', 'com-search_trend_books&page=1');
            }else{
                $myerror->error($add_tip.'System error, add Trend Books failure!', 'BACK');
            }
        }
    }else{
...
if($myerror->getError()){
    ...
    <fieldset>
        <legend class='legend'>Add Trend Books 1.Upload image</legend>
        <?
