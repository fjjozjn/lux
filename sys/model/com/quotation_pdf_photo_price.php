...
//20121212
//gzip和这个页面有冲突，但是我也不知道是为什么，网上说windows里经常有这种情况，linux下就好多了，所以这个页面开始是关掉，结束是再打开，就不会影响其他页面了
//关闭gzip
//20121224 用了global_pdf.php 后就不要用下面这个了
//ob_clean();
...
if(isset($_GET['qid']) && $_GET['qid'] != ''){
	...
	$rs2 = $mysql->q('select pid, photos, price from quote_item where qid = ?', $_GET['qid']);
	if($rs2){
		$result2 = $mysql->fetch();	
	}else{
		die('Error(2)');	
...
for($i = 0; $i < count($result2); $i++){
	...
	$html .= '<td width="33%">'.$img_html.'<br /><span style="float:left">'.$result2[$i]['pid'].'</span>'.'&nbsp;&nbsp;&nbsp;&nbsp;'.'<span style="float:right">USD $'.formatMoney(floatval($result2[$i]['price'])).'</span></td>';
	if($sign == $col_num){
		$html .= '</tr><br />';
		$sign = 0;	
	}
