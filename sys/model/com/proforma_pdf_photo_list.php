...
if(isset($_GET['pvid']) && $_GET['pvid'] != ''){
	$result1 = $mysql->qone('select pvid, send_to, attention, printed_date, printed_by from proforma where pvid = ?', $_GET['pvid']);
	if(!$result1){
		die('Error(1)');	
	}
	...
	$rs2 = $mysql->q('select pid, photos from proforma_item where pvid = ?', $_GET['pvid']);
	if($rs2){
		$result2 = $mysql->fetch();	
	}else{
		die('Error(2)');	
