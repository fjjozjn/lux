...
if($myerror->getWarn()){
    ...
    $formItems = array(
        ...
        'transfer_from' => array('title' => 'Transfer From', 'type' => 'select', 'options' => get_warehouse_info(1), 'required' => 1, 'value' => isset($_GET['wh_id'])?$_GET['wh_id']."|".$_GET['wh_name']:''),
        //转移到新的仓库
        ...
        'q_pid' => array('type' => 'text', 'minlen' => 1, 'maxlen' => 50, 'addon' => 'onblur="itf_pid_blur(this)"', 'value' => isset($_GET['pid'])?$_GET['pid']:''),
        ...
        if($tran_from_wh_id != $tran_to_wh_id){
            ...
            if($itf_item_num >= 1){
                ...
                $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                    ...
                    , ACTION_LOG_SYS_ADD_ITEM_TRANSFER_FORM, $_SESSION["logininfo"]["aName"]." <i>add item transfer form</i> '".$itf_id."' in sys", ACTION_LOG_SYS_ADD_ITEM_TRANSFER_FORM_S, "", "", 0);
