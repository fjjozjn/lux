...
judgeUserPerm( (isset($_GET['inid'])?$_GET['inid']:'') );
...
if($myerror->getWarn()){
    ...
    if( isset($_GET['inid']) && $_GET['inid'] != '' ){
        $mod_result = $mysql->qone('SELECT * FROM warehouse_item_unique WHERE id = ?', $_GET['inid']);
    ...
    if(!$myerror->getAny() && $goodsForm->check()){
        ...
        //总数量增加
        $rs = $mysql->q('update warehouse_item_unique set qty = qty + ?, arrival_date = ?, mod_date = ?, mod_by = ?, remark = ? where id = ?', $qty, $arrival_date, $mod_date, $mod_by, $remark, $_GET['inid']);
        if($rs){
            $mysql->sp('CALL admin_log_insert('.moreQm(8).')', 0, $ip_real, WAREHOUSE_LOG_TYPE, $mod_by." update warehouse <i>".$wh_name."</i> item <i>+ ".$qty.' '.$pid."</i> <i>success(2)</i>", WAREHOUSE_ITEM_UPDATE_SUCCESS, "", "", 0);
            ...
            if($result){
                $mysql->sp('CALL admin_log_insert('.moreQm(8).')', 0, $ip_real, WAREHOUSE_LOG_TYPE, $mod_by." insert warehouse <i>".$wh_name."</i> item <i>log + ".$qty.' '.$pid."</i> <i>success(2)</i>", WAREHOUSE_ITEM_LOG_INSERT_SUCCESS, "", "", 0);
            }else{
                $mysql->sp('CALL admin_log_insert('.moreQm(8).')', 0, $ip_real, WAREHOUSE_LOG_TYPE, $mod_by." insert warehouse <i>".$wh_name."</i> item <i>log + ".$qty.' '.$pid."</i> <i>failure(2)</i>", WAREHOUSE_ITEM_LOG_INSERT_FAILURE, "", "", 0);
            }

            $myerror->ok('Modify Warehouse Item success !', 'com-search_warehouse_item_unique&page=1&wh_name='.$wh_name);
        }else{
            $mysql->sp('CALL admin_log_insert('.moreQm(8).')', 0, $ip_real, WAREHOUSE_LOG_TYPE, $mod_by." update warehouse <i>".$wh_name."</i> item <i>+ ".$qty.' '.$q_pid."</i> <i>failure(2)</i>", WAREHOUSE_ITEM_UPDATE_FAILURE, "", "", 0);
    ...
    if($myerror->getError()){
        ...
        ?>
        <h1 class="green">In Warehouse Item<em>* item must be filled in</em></h1>

        <fieldset class="center2col" style="width:60%">
            <legend class='legend'>In Warehouse Item</legend>
            <?php
            $goodsForm->begin();
            ?>
            <table width="100%">
