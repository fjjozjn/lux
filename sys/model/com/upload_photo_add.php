...
*/
	...
	if ($_FILES["photo"]["error"] > 0){
		...
		if($_FILES['photo']['type'] != 'image/jpg' && $_FILES['photo']['type'] != 'image/jpeg' && $_FILES['photo']['type'] != 'image/gif' && $_FILES["photo"]["type"] != 'image/pjpeg' && $_FILES['photo']['type'] != 'image/png'){
            echo ("<script type='text/javascript'>alert('上传图片 失败! 请使用JPG、PNG或者GIF格式的图片!');history.go(-1);</script>");
            exit();
		}
		echo "Size: " . ($_FILES["photo"]["size"] / 1024) . " Kb<br />";
        //20130610 限制最大500KB
        if(($_FILES["photo"]["size"] / 1024) > 500 ){
            echo ("<script type='text/javascript'>alert('上传图片 失败! 图片大小请不要超过 500 KB!');history.go(-1);</script>");
            exit();
        }
	...
	//if(file_exists(iconv('UTF-8','GBK', $_FILES['photo']['name']))){
		...
		if(file_exists(iconv('UTF-8','GBK', $pic_path_com.$target))) { 
			...
			$_SESSION['upload_photo_add'] = $target;
			echo 'path：'.$pic_path_com.$_SESSION['upload_photo_add'];
			
			$myerror->ok('上传图片 成功!', 'com-addproduct');
		} else { 
			$myerror->error('上传图片 失败!', 'com-addproduct');
		} 
	//}else{
		//$myerror->error('图片名已存在，上传图片 失败!', 'com-addproduct');
	//}
} 
...
<form action="?act=com-upload_photo_add&action=upfile" method="post" name="UForm" enctype="multipart/form-data" onSubmit="return check();"> 
<fieldset> 
