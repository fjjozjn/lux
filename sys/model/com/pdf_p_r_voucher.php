...
if(isset($_GET['prv_id']) && $_GET['prv_id'] != ''){
    //判断是否有访问权限
    /*
    if(!isSysAdmin()){
    ...
    */
    ...
    $result1 = $mysql->qone('select * from sys_p_r_voucher where id = ?', $_GET['prv_id']);
    if(!$result1){
        die('Error(1)');
    }
    ...
    $rs2 = $mysql->q('select * from sys_p_r_voucher_item where main_id = ?', $_GET['prv_id']);
    if($rs2){
        $result2 = $mysql->fetch();
    }
    ...
    $html .= '<table align="center" cellpadding="1" cellspacing="1">
				...
				<tr>
					<td width="30%"></td>
					<td width="40%">PAYMENT / RECEIVED VOUCHER</td>
					<td width="15%"></td>
					<td width="15%"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width="40%"></td>
					<td width="15%">Date.</td>
					<td width="15%">'.$result1['in_date'].'</td>
				</tr>
				<tr>
					<td width="30%"></td>
					<td width="40%"></td>
    ...
    if($rs2){
        ...
        foreach($result2 as $v){
            $html .= '<tr>
                <td width="30%">'.$v['account_name'].'</td>
                <td width="40%">'.$v['description'].'</td>
                <td width="15%">'.$v['dr_currency'].' '.$v['dr'].'</td>
                <td width="15%">'.$v['cr_currency'].' '.$v['cr'].'</td>
            </tr>';
            if($v['dr_currency'] != ''){
                $dr_total_currency = $v['dr_currency'];
            }
            $dr_total += $v['dr'];
            if($v['cr_currency'] != ''){
                $cr_total_currency = $v['cr_currency'];
            }
            $cr_total += $v['cr'];
        }
    ...
    $html .= '<table align="center" cellpadding="1" cellspacing="1"><tr>
                ...
                <td width="15%">'.$dr_total_currency.' '.my_formatMoney($dr_total).'</td>
                <td width="15%">'.$cr_total_currency.' '.my_formatMoney($cr_total).'</td>
            </tr></table>';
    ...
    $pdf->Output($result1['prv_id'].'.pdf', 'I');
