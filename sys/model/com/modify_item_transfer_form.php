...
if($myerror->getWarn()){
    ...
    if(isset($_GET['delid']) && $_GET['delid'] != ''){
        ...
        $del_result = $mysql->qone('SELECT * FROM warehouse_item_transfer_form WHERE id = ?', $_GET['delid']);
        ...
        for($j = 0; $j < $del_itf_item_num; $j++){
            ...
            $mysql->q("update warehouse_item_unique set qty = qty + ?, mod_date = ?, mod_by = ? where wh_id = ? and pid = ?", $del_itf_item_rtn[$j]['qty'], $today, $staff, $del_result['trans_from_wh_id'], $del_itf_item_rtn[$j]['pid']);
            //transfer to的item总量减少
            $mysql->q("update warehouse_item_unique set qty = qty - ?, mod_date = ?, mod_by = ? where wh_id = ? and pid = ?", $del_itf_item_rtn[$j]['qty'], $today, $staff, $del_result['trans_to_wh_id'], $del_itf_item_rtn[$j]['pid']);
        }
        ...
        if($rtn2){
            ...
            $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                ...
                , ACTION_LOG_SYS_DEL_ITEM_TRANSFER_FORM, $_SESSION["logininfo"]["aName"]." <i>delete item transfer form</i> '".$_GET['delid']."' success in sys", ACTION_LOG_SYS_DEL_ITEM_TRANSFER_FORM_S, "", "", 0);
            ...
            $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                , $_SESSION['logininfo']['aID'], $ip_real
                , ACTION_LOG_SYS_DEL_ITEM_TRANSFER_FORM, $_SESSION["logininfo"]["aName"]." <i>delete item transfer form</i> '".$_GET['delid']."' failure in sys", ACTION_LOG_SYS_DEL_ITEM_TRANSFER_FORM_F, "", "", 0);
        ...
        if(!$myerror->getAny() && $goodsForm->check()){
            ...
            if($tran_from_wh_id != $tran_to_wh_id){
                ...
                if($itf_item_num >= 1){
                    ...
                    $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                        , $_SESSION['logininfo']['aID'], $ip_real
                        , ACTION_LOG_SYS_MOD_ITEM_TRANSFER_FORM, $_SESSION["logininfo"]["aName"]." <i>modify item transfer form</i> '".$_GET['modid']."' in sys", ACTION_LOG_SYS_MOD_ITEM_TRANSFER_FORM_S, "", "", 0);
