...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    if(!$rs){
        ...
        if($rs){
            ...
            for($i = 0; $i < $item_num; $i++){
                //20140302 更新invoice表的packing_num字段为此PL ID，以便能在modify invoice的时候自动显示出PL ID ********
                $rtn_vi = $mysql->qone('select packing_num from invoice where vid = ?', $item[$i]['hidden_ref_no']);
                if(isset($rtn_vi['packing_num']) && $rtn_vi['packing_num'] != ''){
                    if(strpos($rtn_vi['packing_num'], $pl_id) === false){
                        $mysql->q('update invoice set packing_num = ? where vid = ?', ($rtn_vi['packing_num'].','.$pl_id),
                            $item[$i]['hidden_ref_no']);
                    }
                }else{
                    $mysql->q('update invoice set packing_num = ? where vid = ?', $pl_id,
                        $item[$i]['hidden_ref_no']);
                }
                ...
                //20140629 更新shipment表
                @$itpl_shipment[$item[$i]['hidden_ref_no']] += $item[$i]['qty'];
            }
            ...
            //20150123
            $hints = '';
            foreach($itpl_shipment as $key=>$v){
            ...
            $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                ...
                , ACTION_LOG_SYS_ADD_PACKING_LIST_FROM_INVOICE, $_SESSION["logininfo"]["aName"]." <i>add packing list</i> '".$pl_id."' from invoice in sys", ACTION_LOG_SYS_ADD_PACKING_LIST_FROM_INVOICE_S, "", "", 0);
...
if($myerror->getError()){
    ...
    <table width="85%" align="center">
        ...
        <tr class="formtitle">
            ...
            <td width="25%"><? $goodsForm->show('vid');?></td></td>
...
if(isset($_GET['vid']) && $_GET['vid'] != ''){
    ?>
    <script>$(function(){addPackingList()})</script>
<?
}
?>