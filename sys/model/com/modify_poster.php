...
if(isset($_GET['delid']) && $_GET['delid'] != ''){
    ...
    $p = $mysql->qone('select photo from poster where id = ?', $_GET['delid']);
    @unlink($pic_path_com_poster.$p['photos']);
    @unlink($pic_path_small_poster.'s_'.$p['photos']);
    @unlink($pic_path_small_poster.'m_'.$p['photos']);
    @unlink($pic_path_small_poster.'l_'.$p['photos']);
    @unlink($pic_path_watermark.'l_water_'.$p['photos']);

    $rtn = $mysql->q('delete from poster where id = ?', $_GET['delid']);
    if($rtn){
        $myerror->ok('删除Poster资料 ('.$_GET['delid'].') 成功!', 'com-search_poster&page=1');
    }else{
        $myerror->error('删除Poster资料 ('.$_GET['delid'].') 失败', 'com-search_poster&page=1');
    }
}else{
    if(isset($_GET['modid']) && $_GET['modid'] != ''){
        $mod_result = $mysql->qone('SELECT * FROM poster WHERE id = ?', $_GET['modid']);
    }

    // form加这个参数才能上传文件，否则获取不到 $_FILE
    $goodsForm = new My_Forms(array('multipart'=>true));
    $formItems = array(
        'poster_date' => array('title' => 'Date', 'type' => 'text', 'restrict' => 'date', 'required' => 1, 'value' => isset($mod_result['poster_date'])?substr($mod_result['poster_date'], 0, 10):''),
        ...
        if( ((@$_FILES['photo']['type'] == 'image/jpg' || @$_FILES['photo']['type'] == 'image/jpeg' || @$_FILES['photo']['type'] == 'image/gif' || @$_FILES['photo']['type'] == 'image/pjpeg' || @$_FILES['photo']['type'] == 'image/png') && (@$_FILES['photo']['size'] / 1024) <= 500) || @$_FILES['photo']['name'] == '' ){
            ...
            if (@$_FILES['photo']['error'] > 0 && @$_FILES['photo']['error'] != 4){
                ...
                if(file_exists(iconv('UTF-8','GBK', $pic_path_com_poster.$target))) {
                    ...
                    //小图片不存在才进行缩小操作
                    if (!is_file($pic_path_small_poster . $small_photo) == true) {
                ...
                if(isset($_GET['modid'])){
                    ...
                    $result = $mysql->q('update poster set photo = ?, poster_date = ?, mod_by = ?, mod_date = ? where id = ?', $photo, $poster_date, $mod_by, $mod_date, $mod_result['id']);
                    //這裡是因為result為0的時候就是update數據和原來一樣，所以判斷時用false
                    if($result !== false){
                        $myerror->ok($add_tip.'Modify poster ('.$mod_result['id'].') success!', 'com-search_poster&page=1');
                    }else{
                        $myerror->error($add_tip.'System error, modify poster ('.$mod_result['id'].') failure!', 'BACK');
                    }
                }
            }
        }else{
...
if($myerror->getError()){
    ...
    <fieldset>
        <legend class='legend'>Modify Poster</legend>
        <fieldset>
            ...
            //20130716
            if(is_file($pic_path_com_poster . $mod_result['photo']) == true){
                $arr = getimagesize($pic_path_com_poster . $mod_result['photo']);
                $pic_width = $arr[0];
                $pic_height = $arr[1];
                //20121022 获取文件大小，在图片旁显示文件的属性
                $a = filesize($pic_path_com_poster . $mod_result['photo']);
                $image_size = getimgsize(100, 60, $pic_width, $pic_height);

                if(isset($_GET['modid'])){
                    echo 'image info：('.$mod_result['photo'].'&nbsp;&nbsp;'.floatval($a/1000).'KB&nbsp;&nbsp;'.$pic_width.'x'.$pic_height.')<div class="shadow" style="margin-left:28px;"><ul><li><a href="/sys/'.$pic_path_com_poster . $mod_result['photo'].'" class="tooltip2" target="_blank" title="'.$mod_result['photo'].'"><img src="/sys/'.$pic_path_com_poster . $mod_result['photo'].'" border="0" align="middle" width="'.$image_size['width'].'" height="'.$image_size['height'].'"/></a></li></ul></div><div><b><a class="button" href="?act=com-delete_photo_new&poster_id='.$mod_result['id'].'&photo='.$mod_result['photo'].'">DELETE</a></b></div>';
                }elseif(isset($_GET['copypid'])){//copy的时候，删了图片，就连原来的product的图片也空了，所以这里去掉了
                    echo 'image info：(将会复制此图片并重新命名&nbsp;&nbsp;'.floatval($a/1000).'KB&nbsp;&nbsp;'.$pic_width.'x'.$pic_height.')<div class="shadow" style="margin-left:28px;"><img src="/sys/'.$pic_path_com_poster . $mod_result['photo'].'" border="0" align="middle" width="'.$image_size['width'].'" height="'.$image_size['height'].'"/></div>';
                }
            }else{
