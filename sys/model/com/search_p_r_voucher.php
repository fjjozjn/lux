...
// 如果有post资料则给Session，并且清除附在上次翻页时残留的$_GET['page']
    ...
    $formItems = array(
        'prv_id' => array(
            'type' => 'text',
            'value' => @$_SESSION['search_criteria']['prv_id'],
        ),
        'created_by' => array(
            'type' => 'text',
    ...
    ?>
    <h1 class="green">P/R Voucher<em>* indicates required fields</em></h1>
            ...
            <td align="center">
                <fieldset>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td align="right">P/R Voucher NO. : </td>
                            <td align="left"><? $form->show('prv_id'); ?></td>
    ...
    if ($getAnyPost || isset($_GET['page'])){
        ...
        $rs->addnew_link = "?act=com-search_p_r_voucher";
        $rs->display_new_button = false;
        $rs->sort_field = "prv_id";
        $rs->sort_seq = "DESC";
        ...
        if (strlen(@$_SESSION['search_criteria']['prv_id'])){
            $where_sql.= " AND prv_id Like '%".$_SESSION['search_criteria']['prv_id'].'%\'';
        }
        ...
        $temp_table = ' sys_p_r_voucher';
        $list_field = ' SQL_CALC_FOUND_ROWS id, prv_id, in_date, mod_date, created_by, mod_by ';

        $info = $mysql->sp('CALL backend_list_withfield(?, ?, ?, ?, ?)', $start_row, $end_row, $temp_table, $where_sql, $list_field);

        $rs->SetRecordCol("P/R Voucher ID", "prv_id");
        $rs->SetRecordCol("Created by", "created_by");
        $rs->SetRecordCol("Date", "in_date");

        $sort = GENERAL_NO;
        $edit = GENERAL_YES;
        $rs->SetRecordCol("PDF", "id", $sort, $edit,"model/com/pdf_p_r_voucher.php?pdf=1", "prv_id");
        $rs->SetRecordCol("MODIFY", "id", $sort, $edit,"?act=com-modify_p_r_voucher", "modid");
        $rs->SetRecordCol("DEL", "id", $sort, $edit,"?act=com-modify_p_r_voucher", "delid");
        $rs->SetRSSorting('?act=com-search_p_r_voucher');
