...
$formItems = array(
    ...
    'color_chart_date' => array('title' => 'Date', 'type' => 'text', 'restrict' => 'date', 'required' => 1, 'value' => date('Y-m-d')),
...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    if( ((@$_FILES['photo']['type'] == 'image/jpg' || @$_FILES['photo']['type'] == 'image/jpeg' || @$_FILES['photo']['type'] == 'image/gif' || @$_FILES['photo']['type'] == 'image/pjpeg' || @$_FILES['photo']['type'] == 'image/png') && (@$_FILES['photo']['size'] / 1024) <= 500) || @$_FILES['photo']['name'] == '' ){
        ...
        if (@$_FILES['photo']['error'] > 0 && @$_FILES['photo']['error'] != 4){
            ...
            $result = $mysql->q('insert into color_chart set photo = ?, color_chart_date = ?, created_by = ?, mod_by = ?, in_date = ?, mod_date = ?', $photos, $color_chart_date, $created_by, $mod_by, $in_date, $mod_date);
            if($result){
                $myerror->ok($add_tip.'Add Color Chart success!', 'com-search_color_chart&page=1');
            }else{
                $myerror->error($add_tip.'System error, add Color Chart failure!', 'BACK');
            }
        }
    }else{
...
if($myerror->getError()){
    ...
    <fieldset>
        <legend class='legend'>Add Color Chart 1.Upload image</legend>
        <?
