...
if($myerror->getWarn()){
    ...
    $formItems = array(
        ...
        'search_by' => array(
            'type' => 'select',
            'options' => array(array("Creation Date", "1"), array("ETD", "2")/*, array("Actual", "3")*/),//Actual以packing list来算，暂时未做
            ...
            <td align="center">
                <fieldset>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
                        <tr>
                            <td height="35">Staff <span style="color: red;">*</span> : </td>
                            <td><? $form->show('staff'); ?></td>
                        </tr>
                        <tr>
            ...
            <td align="center" width="80%">
                ...
                if (strlen(@$_SESSION['search_criteria']['staff'])){
                    ...
                    if($_SESSION['search_criteria']['search_by'] == 1){
                        $rs = $mysql->q('select total, mark_date from proforma where printed_by = ? and istatus <> ? order by mark_date', $_SESSION['search_criteria']['staff'], 'delete');
                        if($rs){
                            $rtn = $mysql->fetch();
                            $proforma_total = array();
                            foreach($rtn as $v){
                                $the_mark_date = explode('-', $v['mark_date']);
                                @$proforma_total[$the_mark_date[0]][intval($the_mark_date[1])] += $v['total'];
                            }
                        }

                    }elseif($_SESSION['search_criteria']['search_by'] == 2){
                        $rs = $mysql->q('select total, expected_date as mark_date from proforma where printed_by = ? and istatus <> ? order by expected_date', $_SESSION['search_criteria']['staff'], 'delete');
                        if($rs){
                            $rtn = $mysql->fetch();
                            $proforma_total = array();
                            foreach($rtn as $v){
                        ...
                        //按packing list来统计
                        /*
                        $p_rs = $mysql->q('select reference from purchase where sid = ? and istatus <> ? group by reference', $_SESSION['search_criteria']['staff'], 'delete');
                        if($p_rs){
                            $p_rtn = $mysql->fetch();
                            $invoice = array();
                            foreach($p_rtn as $v){
                        ...
                        */
                        ...
                        $p_rs = $mysql->q('select pcid from purchase where sid = ? and istatus <> ? order by pcid', $_SESSION['search_criteria']['staff'], 'delete');
                        if($p_rs){
                            $p_rtn = $mysql->fetch();
                            $proforma_total = array();
                            foreach($p_rtn as $v){
                    ...
                    if(isset($proforma_total) && !empty($proforma_total)){
                        ?>
                        <fieldset>
                            <legend class='legend'>
                                <?php
                                if($_SESSION['search_criteria']['search_by'] == 3){
                                    ...
                                    echo 'Information (Monthly Total Amount of Proforma except "delete" status)';
                                }
                                ?>
                            </legend>
                            <div><?=$_SESSION['search_criteria']['staff'];?></div>
                            <br />
                            <table width="100%" border='1' bordercolor='#ABABAB' cellspacing='1' cellpadding='3' bgcolor='#000000' align="center">
                                ...
                                for($i = 2011; $i <= (date('Y')+1); $i++){
                                    ...
                                    $all_total = 0;
                                    for($j = 1; $j <= 12; $j++){
                                        $all_total += @$proforma_total[$i][$j];
                                        echo "<td>".((isset($proforma_total[$i][$j]) && $proforma_total[$i][$j])?formatMoney($proforma_total[$i][$j]):'')."</td>";
                                    }
