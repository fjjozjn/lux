...
if($myerror->getWarn()){
    ...
    if(isset($_GET['delid']) && $_GET['delid'] != ''){
        ...
        $formItems = array(
            'fty_id' => array('type' => 'select', 'options' => get_pcid(), 'required' => 1, 'nostar' => true, 'value' => isset($mod_result['pcid'])?$mod_result['pcid']:''),
            ...
            for($i = 0; $i < $qc_item_num; $i++){
                ...
                if(!empty($photo_array) && $qc_item_rtn[$i]['photo'] != ''){
                    foreach($photo_array as $v){
                        $photo .= '<div><a href="/sys/'.$pic_path_qc_normal.$v.'" target="_blank"><img src="/sys/'.$pic_path_qc_normal.$v.'" width="100" height="80" /></a><input type="file" name="photo_'.$i.'[]" /><img style="cursor: pointer; width: 16px;" src="../images/del_small.png" onclick="del_upload_photo(this)" required=1 /></div>';
                    }
                }
                ?>
                <tr class="formtitle">
