...
if(isset($_GET['vid']) && $_GET['vid'] != ''){
	$result1 = $mysql->qone('select vid, send_to, attention, printed_date, printed_by from invoice where vid = ?', $_GET['vid']);
	if(!$result1){
		die('Error(1)');	
	}
	...
	$rs2 = $mysql->q('select pid, photos from invoice_item where vid = ?', $_GET['vid']);
	if($rs2){
		$result2 = $mysql->fetch();	
	}else{
		die('Error(2)');	
	...
	$html .= '<hr height="2"><table align="left" cellpadding="1" cellspacing="1">
				<tr>
					<td width="15%" rowspan="3">TO: &nbsp;</td>
					<td width="43%" rowspan="3">'.$send_to.'</td>
