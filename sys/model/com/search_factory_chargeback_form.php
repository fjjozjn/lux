...
// print_r_pre($temp_grp);
    ...
    $formItems = array(
        'fcb_id' => array(
            'type' => 'text',
            'value' => @$_SESSION['search_criteria']['fcb_id'],
        ),
        'staff' => array(
            'type' => 'text',
...
// print_r_pre($GLOBALS);
    ?>
    <h1 class="green">Factory Chargeback Form<em>* indicates required fields</em></h1>
    ...
    if ($getAnyPost || isset($_GET['page'])){
        ...
        $rs->addnew_link = "?act=com-search_factory_chargeback_form";
        ...
        if (strlen(@$_SESSION['search_criteria']['fcb_id'])){
            $where_sql.= " AND fcb_id Like '%".$_SESSION['search_criteria']['fcb_id'].'%\'';
        }
        ...
        $temp_table = ' fty_chargeback_form';
        ...
        //$rs->col_width = "100";
        $rs->SetRecordCol("扣款报告编号", "fcb_id");
        $rs->SetRecordCol("工厂", "fty_sid");
        $rs->SetRecordCol("致", "send_to");
        $rs->SetRecordCol("扣款内容原因", "reason");
        $rs->SetRecordCol("扣款金额(RMB)", "money");
        $rs->SetRecordCol("负责同事", "staff");
        $rs->SetRecordCol("日期", "form_date");
        $rs->SetRecordCol("最后修改日期", "mod_date");

        $sort = GENERAL_NO;
        $edit = GENERAL_YES;
        $rs->SetRecordCol("PDF", "fcb_id", $sort, $edit,"model/com/pdf_factory_chargeback_form.php?pdf=1","fcb_id");
        $rs->SetRecordCol("MODIFY", "fcb_id", $sort, $edit, "?act=com-modify_factory_chargeback_form", "modid");
        $rs->SetRecordCol("DEL", "fcb_id", $sort, $edit, "?act=com-modify_factory_chargeback_form", "delid");
        $rs->SetRSSorting('?act=com-search_factory_chargeback_form');
