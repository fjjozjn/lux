...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    if( ((@$_FILES['photo']['type'] == 'image/jpg' || @$_FILES['photo']['type'] == 'image/jpeg' || @$_FILES['photo']['type'] == 'image/gif' || @$_FILES['photo']['type'] == 'image/pjpeg' || @$_FILES['photo']['type'] == 'image/png') && (@$_FILES['photo']['size'] / 1024) <= 500) || @$_FILES['photo']['name'] == '' ){
        ...
        if (@$_FILES['photo']['error'] > 0 && @$_FILES['photo']['error'] != 4){
            ...
            //20170827
            if( (@$_FILES['product_file']['type'] == 'application/pdf' && (@$_FILES['product_file']['size'] / 1024) <= 10240) || @$_FILES['product_file']['name'] == '' ) {

                if (@$_FILES['product_file']['error'] > 0 && @$_FILES['product_file']['error'] != 4){
                    $myerror->error('Upload file Error ! Return Code: '.@$_FILES['product_file']['error'], 'BACK');
                }else {
                    ...
                    if (@$_FILES['photo']['name'] != '') {
                        ...
                        $temp = end(explode('.', @$_FILES['photo']['name']));
                        $target = strtoupper($pid . '_' . $file_date .'.'. $temp);
                        //上传图片
                        ...
                        if (file_exists(iconv('UTF-8', 'GBK', $pic_path_com . $target))) {
                            ...
                            if (isset($_POST['p_show_in_catalog']) && $_POST['p_theme'] == '7') {
                    ...
                    if (@$_FILES['product_file']['name'] != '') {
                        // 转为大写字母(pid + 时间 + 原图片的后缀名)
                        $temp = end(explode('.', @$_FILES['product_file']['name']));
                        $file_target = strtoupper($pid . '_' . $file_date .'.'. $temp);
                        //上传图片
                        move_uploaded_file(@$_FILES['product_file']['tmp_name'], iconv('UTF-8', 'GBK', $product_file_path_com . $file_target));
                        if (file_exists(iconv('UTF-8', 'GBK', $product_file_path_com . $file_target))) {
