...
if($myerror->getWarn()){
    ...
    if(isset($_GET['delid']) && $_GET['delid'] != ''){
        ...
        if(isset($_GET['modid']) && $_GET['modid'] != ''){
            ...
            $sys_purchase = array();
            if(!isSysAdmin()){
                $rs = $mysql->q('select pcid from purchase where sid = ? and istatus = ? order by mark_date desc', $_SESSION['logininfo']['aName'], '(I)');
            }else{
                $rs = $mysql->q('select pcid from purchase where istatus = ? order by mark_date desc', '(I)');
            }
            if($rs){
                $rtn = $mysql->fetch();
                foreach($rtn as $v){
                    $sys_purchase[$v['pcid']] = array($v['pcid'], $v['pcid']);
                }
            }
            ...
            if(isSysAdmin()){
                ...
                //默认都加一个本公司到工厂的客户名单去
                $rs = $mysql->q('select company from fty_client where created_by = ? or created_by = ?', $_SESSION['logininfo']['aName'], 'all');
            }
            if($rs){
                $rtn = $mysql->fetch();
                foreach($rtn as $v){
        ...
        $formItems = array(
            ...
            'fty_id' => array('type' => 'select', 'options' => $sys_purchase),
            'client_company' => array('type' => 'select', 'options' => $client_company, 'value' => isset($mod_result['client_company'])?$mod_result['client_company']:''),
            //'client_address' => array('type' => 'textarea', 'minlen' => 1, 'maxlen' => 100, 'value' => isset($mod_result['client_address'])?$mod_result['client_address']:''),
            'wh_id' => array('type' => 'select', 'options' => get_warehouse_info(1), 'required' => 1, 'value' => isset($mod_result['wh_id'])?($mod_result['wh_id'].'|'.$rtn_wh['wh_name']):'', 'nostar' => true),
        ...
        if(!$myerror->getAny() && $goodsForm->check()){
            ...
            $result = $mysql->q('update delivery set d_date = ?, express_cost = ?, express_id = ?, staff = ?, remark = ?, client_company = ?, client_address = ?, wh_id = ? where d_id = ?', $_POST['d_date'], $_POST['express_cost'], $_POST['express_id'], $_POST['staff'], $_POST['remark'], $_POST['client_company'], $_POST['client_address'], $wh_id, $_GET['modid']);
            ...
            if($result !== false){
                ...
                for($k = 0; $k < $d_item_num; $k++){
                    //找出purchase_item信息
                    $rtn_purchase = $mysql->qone('select price, ccode from purchase_item where pcid = ? and pid = ?', $item[$k]['po_id'], $item[$k]['p_id']);
                    ...
                    if($update_qty != 0){
                        ...
                        $rs_warehouse = $mysql->q('insert into warehouse_item_hist (wh_id, wh_name, pid, action, cost_rmb, qty, photo, arrival_date, in_date, created_by, remark) values ('.moreQm(11).')', $wh_id, $wh_name, $item[$i]['p_id'], $action, $rtn_purchase['price'], abs($update_qty), $rtn_purchase['photos'], $_POST['d_date'], dateMore(), $_SESSION['ftylogininfo']['aName'], 'From modify delivery. '.$_POST['remark']);

                        if($rs_warehouse){
                            if($mysql->q('select id from warehouse_item_unique where wh_id = ? and pid = ?', $wh_id, $item[$i]['p_id'])){
                                if($update_qty > 0){
                                    $mysql->q('update warehouse_item_unique set qty = qty + ?, cost_rmb = ? where wh_id = ? and wh_name = ? and pid = ?', abs($update_qty), $rtn_purchase['price'], $wh_id, $wh_name);
                                }else{
                                    $mysql->q('update warehouse_item_unique set qty = qty - ?, cost_rmb = ? where wh_id = ? and wh_name = ? and pid = ?', abs($update_qty), $rtn_purchase['price'], $wh_id, $wh_name);
                                }
                            }else{
                                $mysql->q('insert into warehouse_item_unique (wh_id, wh_name, pid, description, description_chi, qty, cost_rmb, photo, arrival_date, in_date, created_by, remark) values ('.moreQm(12).')', $wh_id, $wh_name, $item[$i]['p_id'], $rtn_purchase['description'], $rtn_purchase['description_chi'], $item[$i]['quantity'], $rtn_purchase['price'], $rtn_purchase['photos'], $_POST['d_date'], $_SESSION['ftylogininfo']['aName'], 'Add from delivery. '.$_POST['remark']);
                            }
                        }
                    }*/
                }
                ...
                $myerror->ok('修改 出货单 成功!', 'searchdelivery&page=1');
