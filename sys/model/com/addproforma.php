...
$formItems = array(
		...
		'pi_currency' => array('title' => 'Currency', 'type' => 'select', 'options' => get_currency_type(), 'required' => 1),
		'pi_unit' => array('title' => 'Unit', 'type' => 'select', 'options' => get_unit(), 'value' => 'PCS'),
		...
		'pi_expected_date' => array('title' => 'ETD', 'type' => 'text', 'restrict' => 'date', 'required' => 1),
		
		'pi_remark' => array('title' => 'Remark', 'type' => 'text', 'minlen' => 1, 'maxlen' => 200, 'addon' => 'style="width:400px"'),
		'pi_address' => array('title' => 'Address', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 150, 'rows' => 2),
		'pi_remarks' => array('title' => 'Remarks', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 1000, 'rows' => 5),
...
if(!$myerror->getAny() && $goodsForm->check()){
	...
	//remarks 在最後，所以這裡是14，pvid现在是自动生成所以这里变13了，多了ETD所以现在是14了
	foreach( $_POST as $v){
		if( $i <= 14){
			$i++;
		}else{
    ...
    if(!check_repeat_item($pi_pid)){
        ...
        if(!$judge){
            ...
            if($result){
                ...
                $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                    ...
                    , ACTION_LOG_SYS_ADD_PROFORMA, $_SESSION["logininfo"]["aName"]." <i>add proforma</i> '".$pi_pvid."' in sys", ACTION_LOG_SYS_ADD_PROFORMA_S, "", "", 0);
        ...
        $myerror->error('不允许在一个单中添加相同的 Product Item ，新增 Proforma 失败', 'BACK');
    }
}
...
<table width="100%" id="table">
  <tr class="formtitle">
  	<td width="25%"><div class="set"><label class="formtitle">Proforma Invoice NO.</label><br />(autogeneration)</div></td>
