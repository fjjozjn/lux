...
// print_r_pre($GLOBALS);
    ?>
    <h1 class="green">IT Service Request<em>* indicates required fields</em></h1>
    ...
    if ($getAnyPost || isset($_GET['page'])){
        ...
        $rs->addnew_link = "?act=com-search_it_request";
        ...
        $rs->SetRecordCol("Expected Completion Date", "expected_completion_date");
        $rs->SetRecordCol("User", "created_by");
        $rs->SetRecordCol("Date", "in_date");

        $sort = GENERAL_NO;
        $edit = GENERAL_YES;
        //$rs->SetRecordCol("PDF", "qid", $sort, $edit,"model/com/quotation_pdf.php?pdf=1","qid");
        $rs->SetRecordCol("MODIFY", "id", $sort, $edit,"?act=com-modify_it_request","modid");
        $rs->SetRecordCol("DEL", "id", $sort, $edit,"?act=com-modify_it_request","delid");
        $rs->SetRSSorting('?act=com-search_it_request');
