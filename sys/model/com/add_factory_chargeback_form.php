...
$formItems = array(
    ...
    /*'staff' => array('title' => '负责同事', 'type' => 'text', 'required' => 1),*/
    'pcid' => array('title' => 'PO', 'type' => 'select', 'options' => get_pcid(), 'required' => 1),
    'form_date' => array('title' => '日期', 'type' => 'text', 'restrict' => 'date', 'required' => 1),

    'remark' => array('title' => '备注', 'type' => 'text', 'minlen' => 1, 'maxlen' => 100),
    'reason' => array('title' => '扣款内容原因', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 1000),
    'money' => array('title' => '扣款金额(RMB)', 'type' => 'text', 'minlen' => 1, 'maxlen' => 20),
...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    $rs = $mysql->q('insert into fty_chargeback_form set fcb_id = ?, fty_sid = ?, send_to = ?, staff = ?, pcid = ?, form_date = ?, remark = ?, reason = ?, money = ?, in_date = ?, mod_date = ?, created_by = ?, mod_by = ?', $fcb_id, $pc_sid, $pc_attention, $staff, $pcid, $form_date, $remark, $reason, $money, $in_date, $mod_date, $created_by, $mod_by);
    if($rs){
        ...
        $myerror->ok('新增 Factory Chargeback Form 成功!', 'com-search_factory_chargeback_form&page=1');
    }else{
        $myerror->error('新增 Factory Chargeback Form 失败', 'com-add_factory_chargeback_form');
    }
}
