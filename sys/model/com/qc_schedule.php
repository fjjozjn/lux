...
$formItems = array(
    'qcs_date' => array('title' => 'DATE', 'type' => 'text', 'restrict' => 'date', 'required' => 1, 'value' => isset($mod_result['psc_date'])?date('Y-m-d', strtotime($mod_result['psc_date'])):''),
    'pcid' => array('title' => 'PO#', 'type' => 'select', 'options' => get_pcid(), 'required' => 1, 'value' => isset($mod_result['pcid'])?$mod_result['pcid']:''),
...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    //20141203 因现在是自动写入的，所以还是要保持唯一性
    $rtn = $mysql->qone('select id from qc_schedule where pcid = ?', $pcid);
    if( isset($rtn['id']) && $rtn['id'] != '' ){
        $result = $mysql->q('update qc_schedule set qcs_date = ?, pcid = ?, mod_date = ?, mod_by = ? where id = ?', $qcs_date, $pcid, dateMore(), $_SESSION['logininfo']['aName'], $rtn['id']);

        //同时修改po的ETD
        //20150112 不改po的ETD
        //$mysql->q('update purchase set expected_date = ? where pcid = ?', $qcs_date, $pcid);
        ...
        $result = $mysql->q('insert into qc_schedule values (NULL, '.moreQm(6).')', $qcs_date, $pcid, dateMore(), '', $_SESSION['logininfo']['aName'], '');
        if($result){

            //add action log
            $mysql->sp('CALL admin_log_insert(?, ?, ?, ?, ?, ?, ?, ?)'
                , $_SESSION['logininfo']['aID'], $ip_real
                , ACTION_LOG_SYS_ADD_QC_SCHEDULE, $_SESSION["logininfo"]["aName"]." <i>add qc schedule</i> (purchase '".$pcid."') in sys", ACTION_LOG_SYS_ADD_QC_SCHEDULE_S, "", "", 0);
                ...
                foreach($chart_date as $d){
                    ...
                    foreach($rtn as $v){
                        if($d == date('Y-m-d', strtotime($v['qcs_date']))){
                            ...
                            //20150112 检查是否添加了QC_REPORT，如果添加，显示其ID
                            //20151018 移到fty
                            /*$qc_report_rtn = $mysql->qone('select qc_id from qc_report where pcid = ?', $v['pcid']);
                            if(isset($qc_report_rtn['qc_id']) && $qc_report_rtn['qc_id'] != ''){
                                $qc_report_str = "<a href='?act=com-modify_qc_report&modid=".$qc_report_rtn['qc_id']."'>".$qc_report_rtn['qc_id']."</a><br />";
                            }else{
                                $qc_report_str = "<a href='?act=com-add_qc_report&fty_id=".$v['pcid']."'>Add QC Report</a><br />";
                            }*/
                            $qc_report_str = '';
