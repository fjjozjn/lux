...
if(isset($_GET['modid']) && $_GET['modid'] != ''){
	...
	foreach($type as $v){
		if( $v[0] == $mod_result['g_type']){
			$mod_result['g_type'] = $v[1];
			break;
		}
	}
	foreach($material as $v){
		if( $v[0] == $mod_result['g_material']){
			$mod_result['g_material'] = $v[1];
			break;
		}
	}
...
$formItems = array(
		...
		'g_type' => array('title' => '类别', 'type' => 'select', 'options' => $type, 'value' => isset($mod_result['g_type'])?$mod_result['g_type']:''),
		'g_material' => array('title' => '底材用料', 'type' => 'select', 'options' => $material, 'value' => isset($mod_result['g_material'])?$mod_result['g_material']:''),
		...
		'p_plate' => array('title' => '电镀人工价', 'type' => 'text', 'restrict' => 'number', 'minlen' => 1, 'maxlen' => 20, 'value' => isset($mod_result['p_plate'])?$mod_result['p_plate']:''),
		...
		'p_other2' => array('title' => '其他成本', 'type' => 'text', 'restrict' => 'number',  'minlen' => 1, 'maxlen' => 20, 'value' => isset($mod_result['p_other2'])?$mod_result['p_other2']:''),
...
if(!$myerror->getAny() && $goodsForm->check()){
	...
	//}else{
		...
		$people_h = $_SESSION["logininfo"]["aName"];//$_POST['people_h'];
		...
		if(isset($_GET['modid']) && $_GET['modid'] != ''){
			...
			*/
			...
			if($result){
				...
				$_SESSION['upload_photo'] = '';
				$myerror->ok('修改产品资料 成功!', 'formdetail&id='.$_GET['modid']);
			}else{
			...
			*/
			...
			if($result){
				...
				if(is_int($result) && $result > 0){
					...
					@unlink($_SESSION['upload_photo']);
					$_SESSION['upload_photo'] = '';
					$myerror->ok('新增产品资料 成功!', 'formdetail&id='.$result);	
				}else{
...
if(!isset($_GET['modid'])){
	if(isset($_SESSION['upload_photo']) && $_SESSION['upload_photo'] != ''){
		...
		if( $_SESSION['upload_photo'] && $_SESSION['upload_photo'] != ''){
			...
			echo '<div class="shadow" style="margin-left:28px;"><ul><li><a href="/sys/'.$_SESSION['upload_photo'].'" class="tooltip" target="_blank" title="'.$_SESSION['upload_photo'].'"><img src="/sys/'.$_SESSION['upload_photo'].'" border="0" align="middle" width="'.$image_size['width'].'" height="'.$image_size['height'].'"/></a></li></ul></div>';
		}
	}else{
		?>
	...
	if( $mod_result['mysql_photo'] && $mod_result['mysql_photo'] != ''){
		...
		$pic_height = $arr[1];
		$image_size = getimgsize(150, 100, $pic_width, $pic_height);
		echo '<div class="shadow" style="margin-left:28px;"><ul><li><a href="/sys/'.$mod_result['mysql_photo'].'" class="tooltip" target="_blank" title="'.$mod_result['mysql_photo'].'"><img src="/sys/'.$mod_result['mysql_photo'].'" border="0" align="middle" width="'.$image_size['width'].'" height="'.$image_size['height'].'"/></a></li></ul></div>';
	}else{
		?>
		<!--img src="../images/nopic.gif" border="0" height="40"--><div style="margin-left:28px;"><b>【无图片】</b></div>
		<?		
	}
/*
<div class="line"></div>
