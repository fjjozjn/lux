...
if(isset($_GET['id']) || isset($_GET['gid'])){
    if(isset($_GET['id']) && $_GET['id'] != ''){
        ...
        $result = $mysql->qone('select * from bom where g_id = ?', $_GET['gid']);
        $rs_m = $mysql->q('select * from bom_material where bom_id = ?', $result['id']);
        if($rs_m){
            $result_m = $mysql->fetch();
        }
        $rs_t = $mysql->q('select * from bom_task where bom_id = ?', $result['id']);
        if($rs_t){
            $result_t = $mysql->fetch();
        }
    }
    ...
    if($result['photo'] && $result['photo'] != ''){
        $arr = getimagesize(ROOT_DIR.'fty/'.$pic_path_fty_bom.$result['photo']);
        $pic_width = $arr[0];
        $pic_height = $arr[1];
    ...
    <fieldset class="center2col">
        ...
        <table width="1050" border="1">
            <tr height="60">
                ...
                <td colspan="5" rowspan="5" align="center"><img src="<?='/fty/'.$pic_path_fty_bom.$result['photo']?>" align="middle" width="<?=$image_size['width']?>" height="<?=$image_size['height']?>"/></img></td>
            </tr>
            <tr height="60">
                <td><b>类别</b></td>
...
<!--                <td width="70"><b>单价</b></td>-->
            ...
            for($i = 0; $i < 13; $i++){
                ...
                //20150507 公司查看不显示工序的价格
                //20150515 工时和价格都不显示，但是工价总和要算出来显示
        ...
        <p><b>经手人</b>：<?=$result['created_by']?>&nbsp;&nbsp;&nbsp;&nbsp; <b>审核</b>：<?=$result['check_by']?>&nbsp;&nbsp;&nbsp;&nbsp; <b>最后修改时间</b>：<?=$result['g_time']?></p>
    </fieldset>
