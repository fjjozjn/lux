...
*/
	...
	$target_path = 'upload/temp/' . $_FILES['photo']['name']; 
	...
	if(file_exists(iconv('UTF-8','GBK',$target_path))) { 
		...
		//！！！！到時候工廠和公司上傳的圖片要分開放在不同的目錄
		if( isset($_GET['addchg'])){
			if( isset($_SESSION['upload_photo2']) && $_SESSION['upload_photo2'] != ''){
				
			}else{
				
			}
		}elseif( isset($_GET['modchg'])){
			if( isset($_SESSION['modify_photo2']) && $_SESSION['modify_photo2'] != ''){
				
			}else{
				
			}
		}
		$_SESSION['upload_photo2'] = $target_path;
		echo 'path：'.$_SESSION['upload_photo2'];
...
<form action="?act=upload_photo2&action=upfile" method="post" name="UForm" enctype="multipart/form-data" onSubmit="return check();"> 
<fieldset> 
