...
// print_r_pre($temp_grp);
    ...
    $formItems = array(
        'mo_id' => array(
            'type' => 'text',
            'value' => @$_SESSION['search_criteria']['mo_id'],
        ),
        'created_by' => array(
            'type' => 'text',
...
// print_r_pre($GLOBALS);
    ...
    <table width="700" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr>
            <td align="center">
                <fieldset>
                    <legend class='legend'>搜索物料领取单</legend>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
                        <tr>
                            <td>物料领取单编号 : </td>
                            <td><? $form->show('mo_id'); ?></td>
                            <td>领取人 : </td>
    ...
    if ($getAnyPost || isset($_GET['page'])){
        ...
        $rs->addnew_link = "?act=search_material_out";
        ...
        if (strlen(@$_SESSION['search_criteria']['mo_id'])){
            $where_sql.= " AND mo_id Like '%".$_SESSION['search_criteria']['mo_id'].'%\'';
        }
        ...
        $temp_table = ' fty_material_out';
        $list_field = " SQL_CALC_FOUND_ROWS *, id, mo_id, created_by, in_date, date_format(m_out_date, '%Y-%m-%d') as m_out_date";
        ...
        //$rs->col_width = "100";
        $rs->SetRecordCol("物料领取单编号", "mo_id");
        $rs->SetRecordCol("领取人", "created_by");
        $rs->SetRecordCol("日期", "in_date");
        $rs->SetRecordCol("领取日期", "m_out_date");

        $sort = GENERAL_NO;
        $edit = GENERAL_YES;

        $rs->SetRecordCol("修改", "id", $sort, $edit, "?act=modify_material_out", "modid");
        //$rs->SetRecordCol("PDF", "pcid", $sort, $edit, "model/viewpurchase_pdf.php?pdf=1","pcid");
        $rs->SetRecordCol("删除", "id", $sort, $edit, "?act=modify_material_out", "delid");
        $rs->SetRSSorting('?act=search_material_out');
