...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$rtn = $mysql->qone('select sid, name, name_en, address, tel from supplier where sid = ?', $rtn_delivery['sid']);
...
if(isset($_GET['d_id']) && $_GET['d_id'] != ''){
	...
	$mod_item_result = $mysql->q('select * from delivery_item where d_id = ?', $mod_result['d_id']);
	if($mod_item_result){
	...
	//找到調高度的方法了
	$pdf->Ln(1);
	$html = '<span align="center"><b>送货单</b><span/>';
...
$html .= '<div class="line"></div>
    <table width="100%" border="0.5"  align="left">    
        ...
        <tbody id="tbody" class="delivery" align="center">';
			...
			$html .= '</tbody>';
			$html .= '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td align="right">运费：</td><td align="right">'.formatMoney($mod_result['express_cost']).'</td><td></td><td align="right">运单编号：</td><td align="center">'.$mod_result['express_id'].'</td></tr>';
