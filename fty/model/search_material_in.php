...
// print_r_pre($temp_grp);
    ...
    $formItems = array(
        'mi_id' => array(
            'type' => 'text',
            'value' => @$_SESSION['search_criteria']['mi_id'],
        ),
        'created_by' => array(
            'type' => 'text',
...
// print_r_pre($GLOBALS);
    ...
    <table width="700" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr>
            <td align="center">
                <fieldset>
                    <legend class='legend'>搜索物料存仓单</legend>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
                        <tr>
                            <td>物料存仓单编号 : </td>
                            <td><? $form->show('mi_id'); ?></td>
                            <td>存仓人 : </td>
    ...
    if ($getAnyPost || isset($_GET['page'])){
        ...
        $rs->addnew_link = "?act=search_material_in";
        ...
        if (strlen(@$_SESSION['search_criteria']['mi_id'])){
            $where_sql.= " AND mi_id Like '%".$_SESSION['search_criteria']['mi_id'].'%\'';
        }
        ...
        $temp_table = ' fty_material_in';
        $list_field = " SQL_CALC_FOUND_ROWS *, id, mi_id, created_by, in_date, date_format(m_in_date, '%Y-%m-%d') as m_in_date";
        ...
        //$rs->col_width = "100";
        $rs->SetRecordCol("物料存仓单编号", "mi_id");
        $rs->SetRecordCol("存仓人", "created_by");
        $rs->SetRecordCol("日期", "in_date");
        $rs->SetRecordCol("存仓日期", "m_in_date");
        ...
        $rs->SetRSSorting('?act=search_material_in');
