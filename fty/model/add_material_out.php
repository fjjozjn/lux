...
$formItems = array(
    ...
    'm_out_date' => array('title' => '领取日期', 'type' => 'text', 'restrict' => 'date', 'required' => 1),
...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    $judge = $mysql->q('select id from fty_material_out where mo_id = ?', $mo_id);
    if(!$judge){
        $result = $mysql->q('insert into fty_material_out set mo_id = ?, m_out_date = ?, remark = ?, in_date = ?, mod_date = ?, created_by = ?, mod_by = ?', $mo_id, $m_out_date, $remark, $today, $today, $staff, $staff);

        if($result){
            foreach($material_arr as $v){
                $rtn = $mysql->qone('select m_name, m_type, m_color, m_unit, m_loss from fty_material where m_id = ?', $v['m_id']);
                $mysql->q('insert into fty_material_out_item set main_id = ?, mo_type = ?, mo_id = ?, mo_name = ?, mo_color = ?, mo_unit = ?, mo_price = ?, mo_value = ?, mo_total = ?, mo_remark = ?', $result, $rtn['m_type'], $v['m_id'], $rtn['m_name'], $rtn['m_color'], $rtn['m_unit'], $v['price'], $v['value'], round($v['price']*$v['value']*(1+$rtn['m_loss']/100), 2), $v['remark']);
                //更改库存
                changeFtyMaterialNum($v['m_id'], -$v['value']);
            }

            $myerror->ok('新增 物料领取单 成功!', 'search_material_out&page=1');
        }else{
            $myerror->error('新增 物料领取单 失败', 'BACK');
        }
    }else{
        $myerror->error('输入的 物料领取单ID 已存在，新增失败', 'BACK');
    }
}
    ...
    <fieldset>
        ...
        <table width="100%" id="table">
            <tr class="formtitle" valign="top">
                <td width="25%"><div class="set"><label class="formtitle">物料领取单编号</label><br />(自动生成)
                    </div></td>
        ...
        <div style="margin-left:28px;">
            <label class="formtitle" for="g_cast"><font size="+1">领取物料</font></label>
