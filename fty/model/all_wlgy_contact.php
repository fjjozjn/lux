...
if($myerror->getWarn()){
    ...
    if (isset($_GET['cid'])){
        ?>
        <h1 class="green">物料供应商联系人 ---- <?php echo $_GET['cid']; ?></h1>

        <fieldset>
            <legend class='legend'>操作</legend>
            <div style="margin-left:28px;"><a class="button" href="?act=add_wlgy_contact&cid=<?php echo $_GET['cid']; ?>"><b>添加联系人</b></a></div>
        </fieldset>
        ...
        $sort = GENERAL_NO;
        $edit = GENERAL_YES;
        $rs->SetRecordCol("修改", "id", $sort, $edit,"?act=c_modify_wlgy_contact","modid");
        $rs->SetRecordCol("删除", "id", $sort, $edit,"?act=c_modify_wlgy_contact","delid");
        $rs->SetRSSorting('?act=all_wlgy_contact');
