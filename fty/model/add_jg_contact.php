...
$formItems = array(
    ...
    'cid' => array('title' => '加工商编号', 'type' => 'select', 'minlen' => 1, 'maxlen' => 20, 'options' => get_fty_jg_customer()),
    ...
    'position' => array('title' => '职位', 'type' => 'textarea', 'minlen' => 1, 'maxlen' => 50),
    ...
    'qq' => array('title' => 'QQ', 'type' => 'text', 'minlen' => 1, 'maxlen' => 50),
    'wechat' => array('title' => 'Wechat', 'type' => 'text', 'minlen' => 1, 'maxlen' => 50),
...
if(!$myerror->getAny() && $goodsForm->check()){
    ...
    $result = $mysql->q('insert into fty_jg_contact (name, family_name, title, address, position, tel1, tel2, fax, email, cid, send_style_list, qq, wechat) values ('.moreQm(13).')', $first_name, $family_name, $title, $address, $position, $tel1, $tel2, $fax, $email, $cid, $send_style_list, $qq, $wechat);
    if($result){
        $myerror->ok('新增 加工商联系人 成功!', 'search_jg_contact&page=1');
    }else{
        $myerror->error('新增 加工商联系人 失败', 'add_jg_contact');
    }
...
if($myerror->getError()){
    ...
    <fieldset>
        <legend class='legend'>添加加工商联系人</legend>
