...
// print_r_pre($GLOBALS);
    ...
    <table width="700" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr>
            <td align="center">
                <fieldset>
                    <legend class='legend'>搜索物料需求单</legend>
    ...
    if ($getAnyPost || isset($_GET['page'])){
        ...
        $rs->addnew_link = "?act=search_fty_material_require";
        ...
        $temp_table = ' fty_material_require';
        $list_field = ' SQL_CALC_FOUND_ROWS *, id, mr_id, pcid, created_by, in_date, require_date';
        ...
        //$rs->col_width = "100";
        $rs->SetRecordCol("物料需求单编号", "mr_id");
        $rs->SetRecordCol("生产单编号", "pcid");
        $rs->SetRecordCol("需求日期", "require_date");
        $rs->SetRecordCol("填写人", "created_by");
        $rs->SetRecordCol("日期", "in_date");

        $sort = GENERAL_NO;
        $edit = GENERAL_YES;

        $rs->SetRecordCol("修改", "id", $sort, $edit, "?act=modify_fty_material_require", "modid");
        //$rs->SetRecordCol("PDF", "pcid", $sort, $edit, "model/viewpurchase_pdf.php?pdf=1","pcid");
        $rs->SetRecordCol("删除", "id", $sort, $edit, "?act=modify_fty_material_require", "delid");
        $rs->SetRSSorting('?act=search_fty_material_require');
