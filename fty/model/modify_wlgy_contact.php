...
if($myerror->getWarn()){
    ...
    if(isset($_GET['delid']) && $_GET['delid'] != ''){
        $rtn = $mysql->q('delete from fty_wlgy_contact where id = ?', $_GET['delid']);
        if($rtn){
            $myerror->ok('删除 物料供应商联系人 成功!', 'search_wlgy_contact&page=1');
        }else{
            $myerror->error('删除 物料供应商联系人 失败!', 'search_wlgy_contact&page=1');
        }
    }else{
        if(isset($_GET['modid']) && $_GET['modid'] != ''){
            $mod_result = $mysql->qone('SELECT * FROM fty_wlgy_contact WHERE id = ?', $_GET['modid']);
        }else{
            die('Need modid!');
        }
        ...
        $formItems = array(
            ...
            'cid' => array('title' => '物料供应商编号', 'type' => 'select', 'minlen' => 1, 'maxlen' => 20, 'options' => get_fty_wlgy_customer(), 'value' => isset($mod_result['cid'])?$mod_result['cid']:''),
        ...
        if(!$myerror->getAny() && $goodsForm->check()){
            ...
            $result = $mysql->q('update fty_wlgy_contact set name = ?, family_name = ?, title = ?, address = ?, position = ?, tel1 = ?, tel2 = ?, fax = ?, email = ?, cid = ?, send_style_list = ? where id = ?', $first_name, $family_name, $title, $address, $position, $tel1, $tel2, $fax, $email, $cid, $send_style_list, $_GET['modid']);
            if($result){
                $myerror->ok('修改 物料供应商联系人 成功!', 'search_wlgy_contact&page=1');
            }else{
                $myerror->error('修改 物料供应商联系人 失败', 'BACK');
            }
        }
    }
    ...
    if($myerror->getError()){
        ...
        <fieldset>
            <legend class='legend'>修改物料供应商联系人</legend>
            <?php
            $goodsForm->begin();
            ?>
            <table width="100%">
