...
*/
	...
	if ($_FILES["photo"]["error"] > 0){
        ...
        if($_FILES['photo']['type'] != 'image/jpg' && $_FILES['photo']['type'] != 'image/jpeg' && $_FILES['photo']['type'] != 'image/gif' && $_FILES["photo"]["type"] != 'image/pjpeg' && $_FILES['photo']['type'] != 'image/png'){
			$myerror->error('上传图片 失败!文件不是JPG、PNG或者GIF图片!', 'modifyproduct');
			die();
		} 
	...
	if(!file_exists(iconv('UTF-8', 'GBK', $pic_path_fty.$_FILES['photo']['name']))){
		...
		if( isset($_GET['chg'])){
			//把旧的删了，以防用户都不知到哪些图片名不能用，用了会重复
			//有可能沒有圖片，转到其他页面图片的session都不见了，还怎么删啊。。。。所以多传了个参数，mod by zjn 2012.4.3
			@unlink(iconv('UTF-8', 'GBK', $pic_path_fty.$_GET['chg']));
		}
		...
		if(file_exists(iconv('UTF-8','GBK', $pic_path_fty.$target))) { 
			...
			$_SESSION['fty_upload_photo_mod'] = $target;
			echo 'path：'.$pic_path_fty.$_SESSION['fty_upload_photo_mod'];
			
			$myerror->ok('上传图片 成功!', 'modifyproduct&modid='.$_SESSION['modid']);
		} else { 
			$myerror->error('上传图片 失败!', 'modifyproduct&modid='.$_SESSION['modid']);
		}
	}else{
		//使用原来上传的图片
		$_SESSION['fty_upload_photo_mod'] = $_FILES['photo']['name'];
		$myerror->warn('同名图片已存在，请返回!', 'modifyproduct&modid='.$_SESSION['modid'], 3);
	}
} 
...
<form action="?act=upload_photo_mod&action=upfile&chg=<?=isset($_GET['chg'])?$_GET['chg']:''?>" method="post" name="UForm" enctype="multipart/form-data" onSubmit="return check();"> 
<fieldset> 
