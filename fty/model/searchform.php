...
$formItems = array(
		...
		'start_total' => array(
			'type' => 'text', 
			'restrict' => 'number',
			'value' => @$_SESSION['search_criteria']['start_total'], 
			),	
		'end_total' => array(
			'type' => 'text', 
			'restrict' => 'number',
			'value' => @$_SESSION['search_criteria']['end_total'], 
			),				
		'g_sample_order_no' => array(
			'type' => 'text', 
			'value' => @$_SESSION['search_criteria']['g_sample_order_no'], 
			),
        'p_status' => array(
            'type' => 'select',
            'options' => array(array('未完成', '未完成'),array('已完成', '已完成')),
            'value' => @$_SESSION['search_criteria']['p_status'],
            ),
        'submitbutton' => array(
			'type' => 'submit', 
			'value' => '搜索', 
	...
	<legend class='legend'>搜索</legend>
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
			...
			<tr>
				<td>板单编号 : </td>
				<td><?
				$form->show('g_sample_order_no');
				?></td>		
                
                <td>状态 ：</td>
				<td><? $form->show('p_status');?></td>
			</tr>            
			<tr>
...
<?
	...
	if ($getAnyPost || isset($_GET['page'])){
		...
		if (strlen(@$_SESSION['search_criteria']['g_ccode'])){
			$where_sql.= " AND g_ccode Like '%".$_SESSION['search_criteria']['g_ccode'].'%\'';
		}	
		if (strlen(@$_SESSION['search_criteria']['g_sample_order_no'])){
			$where_sql.= " AND g_sample_order_no Like '%".$_SESSION['search_criteria']['g_sample_order_no'].'%\'';
		}
        if (strlen(@$_SESSION['search_criteria']['p_status'])){
            $where_sql.= " AND p_status = '".$_SESSION['search_criteria']['p_status']."'";
        }
		...
		if (strlen(@$_SESSION['search_criteria']['start_total'])){
			if (strlen(@$_SESSION['search_criteria']['end_total'])){
				$where_sql.= " AND p_total between ".$_SESSION['search_criteria']['start_total']." AND ".$_SESSION['search_criteria']['end_total'];
			}else{
				$where_sql.= " AND p_total > ".$_SESSION['search_criteria']['start_total'];
			}
		}elseif (strlen(@$_SESSION['search_criteria']['end_total'])){
			$where_sql.= " AND p_total < ".$_SESSION['search_criteria']['end_total'];
		}		
		
		//普通用户只能搜索到自己开的单
		if (!isFtyAdmin()){
			$where_sql .= " AND created_by in (select AdminName from tw_admin where FtyName = (select FtyName from tw_admin where AdminName ='".$_SESSION['ftylogininfo']['aName']."'))";
		}
		...
		$where_sql.= ' ORDER BY g_time DESC ';
		$_SESSION['search_criteria']['page'] = $current_page;

		$temp_table = ' bom';
		$list_field = ' SQL_CALC_FOUND_ROWS id, g_id, g_type, g_time, p_status, p_total, photo, g_sample_order_no, g_ccode, created_by, check_by ';
		...
		$rs->SetRecordCol("客号", "g_ccode");
		$rs->SetRecordCol("类别", "g_type");
		$rs->SetRecordCol("板单编号", "g_sample_order_no");
		$rs->SetRecordCol("总价", "p_total");
		$rs->SetRecordCol("创建人", "created_by");
		$rs->SetRecordCol("最后修改时间", "g_time");
        $rs->SetRecordCol("审核人", "check_by");
