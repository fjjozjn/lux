...
$result = $mysql->qone('select p_status from bom where id = ?', $_GET['id']);
if($result['p_status'] != '已完成'){
	die('未核批！');
}
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$rtn = $mysql->qone('select name from supplier where sid = ?', $_SESSION['ftylogininfo']['aFtyName']);
		//fb($rtn);		
		$this->SetFont('droidsansfallback', '', 20);
		$this->Cell(0, 0, $rtn['name']?$rtn['name']:'Lux 内部使用', 0, 1, 'C', 0, '', 0);
	}	
	...
	public function Footer() {
		...
		// Page number
		$this->Cell(200, 15, $this->getAliasNumPage().' / '.$this->getAliasNbPages().' 页', 0, false, 'R', 0, '', 0, false, 'T', 'M');
	}
}
...
if(isset($_GET['id']) && $_GET['id'] != ''){
	...
	if($result['photo'] && $result['photo'] != ''){
		...
		$img_html = '<img src="/fty/'.$pic_path_fty_bom.$result['photo'].'" border="0" align="middle" width="'.$image_size['width'].'" height="'.$image_size['height'].'"/>';
	}
    //物料总价
    $m_total = 0;
    //工序总价
...
for($i = 0; $i < 13; $i++){
	if(isset($result_m[$i]['m_id']) && $result_m[$i]['m_id'] != ''){
		...
		$html .= '<tr><td>' . $result_m[$i]['m_id'] . '</td><td>' . $result_m[$i]['m_name'] . '</td><td>' . $result_m[$i]['m_color'] . '</td><td>' . $result_m[$i]['m_type'] . '</td><td>' . ($result_m[$i]['m_unit'].':'.$result_m[$i]['m_value']) . '</td><td>' . ($result_m[$i]['m_price']*$result_m[$i]['m_value']*(1+$m_info['m_loss']/100)) . '</td><td>' . $result_m[$i]['m_remark'] . '</td>';
	...
	if(isset($result_t[$i]['t_id']) && $result_t[$i]['t_id'] != ''){
		$html .= '<td>' . $result_t[$i]['t_id'] . '</td><td>' . $result_t[$i]['t_name'] . '</td><td>' . $result_t[$i]['t_time'] . '</td><td>' . ($result_t[$i]['t_price']*$result_t[$i]['t_time']) . '</td><td>' . $result_t[$i]['t_remark'] . '</td></tr>';
...
for($i = 13; $i < 23; $i++){
	if($i == 13){
		if(isset($result_m[$i]['m_id'])){
            $m_info = $mysql->qone('select m_loss from fty_material where m_id = ?', $result_m[$i]['m_id']);
			$html .= '<tr><td>' . $result_m[$i]['m_id'] . '</td><td>' . $result_m[$i]['m_name'] . '</td><td>' . $result_m[$i]['m_color'] . '</td><td>' . $result_m[$i]['m_type'] . '</td><td>' . ($result_m[$i]['m_unit'].':'.$result_m[$i]['m_value']) . '</td><td>' . ($result_m[$i]['m_price']*$result_m[$i]['m_value']*(1+$m_info['m_loss']/100)) . '</td><td>' . $result_m[$i]['m_remark'] . '</td><td colspan="5" rowspan="8">&nbsp;</td></tr>';
            $m_total += ($result_m[$i]['m_price']*$result_m[$i]['m_value']*(1+$m_info['m_loss']/100));
		}else{
...
$html .= '<tr><td colspan="12"><b>物料总价</b>：'.formatNumToMoney($m_total).'&nbsp;&nbsp;<b>工序总价</b>：'.formatNumToMoney($t_total).'&nbsp;&nbsp;<b>电镀人工价</b>：'.formatNumToMoney($result['p_plate']).'&nbsp;&nbsp;<b>其他成本</b>：'.formatNumToMoney($result['p_other']).'&nbsp;&nbsp;<b>利润</b>：'.formatNumToMoney($result['p_profit']).'&nbsp;&nbsp;<b>合计</b>：'.formatNumToMoney($result['p_total']).' </td></tr>';
$html .= '</table>';
$html .= '<br /><b>填写人</b>：'.$result['created_by'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>审核</b>：'.$result['check_by'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>最后修改时间：</b>'.$result['g_time'].'';
