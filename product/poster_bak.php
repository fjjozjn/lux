...
<!--        <link href="css/style.css" rel="stylesheet" type="text/css" />-->
    ...
    <div id="page">
        ...
        <div id="container">
            ...
            <?
            //间距50px
            foreach($poster_date as $poster_date_item){
                echo '<a id="poster_date" href="?poster_date='.$poster_date_item.'">'.$poster_date_item.'</a><span style="width:50px">&nbsp;</span>';
            }
            ?>
            <br />
            <br />
...
<script>
    function change_color(poster_date){
        var now = new Date();
        var this_month = now.getFullYear()+"-"+((now.getMonth()+1)<10?"0":"")+(now.getMonth()+1);
        if(poster_date == '') poster_date = this_month;

        $("[id='poster_date']").each(function(){
            if( $(this).html() == poster_date ){
    ...
    change_color('<?=(isset($_GET['poster_date'])?htmlspecialchars($_GET['poster_date']):'')?>');
