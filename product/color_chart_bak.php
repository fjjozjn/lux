...
//    die('@');
    ...
    if(isset($_GET['color_chart_date']) && $_GET['color_chart_date'] != '') {
        $_SESSION['color_chart_date'] = $_GET['color_chart_date'];
        $sql_color_chart_date = $_GET['color_chart_date'];
    }else{
        $_SESSION['color_chart_date'] = date('Y-m');
        $sql_color_chart_date = date('Y-m');
    }
    $rs = $mysql->q('select id, color_chart_date, photo from color_chart where color_chart_date like ? order by id desc', $sql_color_chart_date.'%');
    $rtn = '';
    if($rs){
        $rtn = $mysql->fetch();
    }
...
//    die('#');
    ...
    <div id="page">
        ...
        <div id="container">
            ...
            <?
            //间距50px
            foreach($color_chart_date as $color_chart_date_item){
                echo '<a id="color_chart_date" href="?color_chart_date='.$color_chart_date_item.'">'.$color_chart_date_item.'</a><span style="width:50px">&nbsp;</span>';
            }
            ?>
            <br />
            <br />
            ...
            if($rs){
                ...
                <div id="thumbs" class="navigation">
                    <ul class="thumbs noscript">
                        <?
                        $img_dir = 'http://'.$host.'/sys/'.$pic_path_com_color_chart;
                        foreach($rtn as $v){
                            if(file_exists(ROOT_DIR.'sys/'.$pic_path_small_color_chart . 's_' . $v['photo'])){
                                $thumb_img_dir = 'http://'.$host.'/sys/'.$pic_path_small_color_chart.'s_';//只能用远程地址，例子中也是这样的
                            }else{
                                $thumb_img_dir = 'http://'.$host.'/sys/'.$pic_path_com_color_chart;
                            }
                            ?>
                            <li>
...
<script>
    function change_color(color_chart_date){
        var now = new Date();
        var this_month = now.getFullYear()+"-"+((now.getMonth()+1)<10?"0":"")+(now.getMonth()+1);
        if(color_chart_date == '') color_chart_date = this_month;

        $("[id='color_chart_date']").each(function(){
            if( $(this).html() == color_chart_date ){
    ...
    change_color('<?=(isset($_GET['color_chart_date'])?htmlspecialchars($_GET['color_chart_date']):'')?>');
