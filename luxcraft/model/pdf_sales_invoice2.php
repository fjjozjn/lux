...
if(isset($_GET['sales_vid']) && $_GET['sales_vid'] != ''){
    ...
    */
    ...
    //20141118 加pdf分页显示
    //第一页显示的item个数（有表头）
    $first_page_item_num = 6;
    //除第一页外的显示item个数（无表头）
    $other_page_item_num = 10;
 ...
 cellspacing="1">
            ...
            for($i = 0; $i < count($result2); $i++){
                ...
                $html .= '<tr>
                        ...
                        <td width="22%">'.number_format($result2[$i]['price']*$result2[$i]['qty'], 2).'</td>
                    </tr>';
            }
            $html .= '</table>';
            ...
            if($result_i == count($result_item)){
                $html .= '<table align="center" cellpadding="1" cellspacing="1">
                    <tr>
                        ...
                        <td width="22%" align="center">'.number_format($result1['discount'], 2).'</td>
                    </tr>
                    <tr>
                        <td width="20%"></td>
                        <td width="25%"></td>
                        ...
                        <td width="22%" align="center">'.number_format($total_amount, 2).'</td>
                    </tr>
                    </table>';
            }
            //$pdf->writeHTML($html, false, false, true, false, '');
        ...
        //shop 信息
        /*
        $rs3 = $mysql->q('select address, address_en from warehouse where type = 2');
        $result3 = $mysql->fetch();
        ...
        foreach($result3 as $v){
            ...
            $shop .= '<td>'.$v['address'].'<br />'.$v['address_en'].'</td>';
            if($i%2 == 0){
                $shop .= '</tr>';
            }
            $i++;
        ...
        */
        ...
        $pdf->writeHTMLCell(150, '', '', 112, $shop, 0, 0, false, true, '', true);
        $pdf->writeHTMLCell(150, '', '', 260, $shop, 0, 0, false, true, '', true);
